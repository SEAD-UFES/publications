...
module.exports = app => {
  ...
  api.list = async (req, res) => {
    ...
    models.SelectiveProcess.findAndCountAll({
      ...
      e => {
        res.status(500).json(error.parse('selectiveProcesses-400', e))
      }
    )
  }
  ...
  api.create = async (req, res) => {
    ...
    try {
      ...
      res.status(400).json(error.parse('selectiveProcesses-500', 'Error during validation'))
    }
    ...
    if (isEmpty(errors)) {
      try {
        ...
        res.status(400).json(error.parse('selectiveProcesses-500', 'Error trying to create new Selective Process.'))
      }
    } else {
      res.status(400).json(error.parse('selectiveProcesses-406', { errors }))
    }
  }
  ...
  api.specific = (req, res) => {
    models.SelectiveProcess.findById(req.params.id, {
      ...
      selectiveProcess => {
        if (!selectiveProcess) {
          res.status(400).json(error.parse('selectiveProcesses-404', {}))
        } else {
      ...
      e => {
        res.status(500).json(error.parse('selectiveProcesses-404', e))
      }
    )
  }
  ...
  api.update = async (req, res) => {
    ...
    try {
      ...
      res.status(400).json(error.parse('selectiveProcesses-500', e))
    }
    ...
    if (isEmpty(errors)) {
      try {
        ...
        res.status(400).json(error.parse('selectiveProcesses-500', 'Error updating Process'))
      }
    } else {
      res.status(500).json(error.parse('selectiveProcesses-406', { errors }))
    }
  }
